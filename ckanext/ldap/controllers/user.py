            ...
            if ldap_user_dict and _check_ldap_password(ldap_user_dict['cn'], password):
                ...
                if config['ckanext.ldap.ckan_fallback']:
                    ...
                    if exists['exists'] and not exists['is_ldap']:
                        return self._login_failed(error=_('Conflito de usuario. Entre em contato com o administrador.'))
                return self._login_failed(error=_('Nome de usuario ou senha incorretos.'))
                ...
                return self._login_failed(error=_('Nome de usuario ou senha incorretos.'))
        return self._login_failed(error=_('Digite seu usuario e senha'))
...
def _get_or_create_ldap_user(ldap_user_dict):
    ...
    if exists['exists'] and not exists['is_ldap']:
        raise UserConflictError(_('Conflito de usuario. Entre em contato com o administrador.'))
